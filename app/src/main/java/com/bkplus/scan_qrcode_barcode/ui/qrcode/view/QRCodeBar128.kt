...
class QRCodeBar128 : BaseCustomViewLinearLayout {
    ...
    private fun setupUI() {
        viewBarCode.setHint(context.getString(R.string.hint_128))
        viewBarCode.edtContent.addTextChangedListener(mTextEditorWatcher)
    }

    fun generateQRCode() {
        if (QRCodeManager.isValid128(viewBarCode.getContent())) {
            val result = QRCodeManager.instance().generateBarCODE128Code(
            ...
            Toast.makeText(
                this.context,
                context.getString(R.string.invalid_form_128),
                Toast.LENGTH_SHORT
            ).show()
            return
        }
