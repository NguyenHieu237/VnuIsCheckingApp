...
class QRCodeBar93 : BaseCustomViewLinearLayout {
    ...
    private fun setupUI() {
        viewBarCode.setHint(context.getString(R.string.hint_93))
    ...
    fun generateQRCode() {
        ...
        if (QRCodeManager.isValidCode39("* $value *")) {
            val result = QRCodeManager.instance().generateBarCODE93Code(
            ...
            Toast.makeText(
                this.context,
                context.getString(R.string.invalid_form_93),
                Toast.LENGTH_SHORT
            ).show()
            return
        }
