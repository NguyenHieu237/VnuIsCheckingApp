...
class QRCodeBarITF: BaseCustomViewLinearLayout {
    ...
    private fun setupUI() {
        ...
        viewBarCode.setHint(context.getString(R.string.hint_itf))
        viewBarCode.edtContent.addTextChangedListener(mTextEditorWatcher)
    }

    fun generateQRCode() {
        if (QRCodeManager.isValidITF(viewBarCode.getContent())) {
            val result = QRCodeManager.instance().generateBarITFCode(
        ...
        {
            Toast.makeText(this.context, context.getString(R.string.invalid_form_ITF), Toast.LENGTH_SHORT).show()
            return
        }
