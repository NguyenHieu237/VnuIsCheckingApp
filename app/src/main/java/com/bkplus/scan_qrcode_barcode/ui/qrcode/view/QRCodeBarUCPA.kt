...
class QRCodeBarUCPA: BaseCustomViewLinearLayout {
    ...
    fun generateQRCode() {
        if (QRCodeManager.isValidEANandUCP(viewBarCode.getContent())) {
            ...
            Toast.makeText(
                this.context,
                context.getString(R.string.invalid_form_upca),
                Toast.LENGTH_SHORT
            ).show()
            return
        }
    ...
    private val mTextEditorWatcher: TextWatcher = object : TextWatcher {
        ...
        override fun onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) {
            if(viewBarCode.edtContent.length() == 12){
